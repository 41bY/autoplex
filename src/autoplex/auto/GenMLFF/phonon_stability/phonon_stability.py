...
# Add FileHandler for logfile
file_handler = logging.FileHandler("local_run.log")
file_handler.setLevel(logging.INFO)
file_handler.setFormatter(logging.Formatter(
    "%(asctime)s - %(name)s - %(levelname)s - %(message)s"
))
logging.getLogger("jobflow").addHandler(file_handler)

@job
def compute_stability_metrics(list_of_pstability_results, neg_modes_threshold=0.01):
    """
    """
...
def local_phonon_flow(
    ...
    for idx_structure, pmg_structure, ph_maker_kwargs, st_maker_kwargs, relax_maker_kwargs in zip(
        ...
        if phonon_dos is not None:
            pos_modes, neg_modes = _compute_dynamic_stability(phonon_dos)
        else:
        ...
        #Remove the phonon DOS and bandstructure from the stability output to save up space
...
def _compute_dynamic_stability(
...
def _dump_stability_output(
    ...
    #Move ase structures
    os.rename(f"{cwd}/final_atoms_object.xyz", f"{output_path}/final_atoms_object.xyz")
    ...
    # #Extract the main results from the stability output
    # structure_id = stability_output["structure_index"]
    # relaxed_structure = stability_output["relaxed_structure"]
    # phonon_dos = stability_output["phonon_dos"]
    # phonon_bs = stability_output["phonon_bandstructure"]

    # #Serialize objects
    # relaxed_structure_doc = relaxed_structure.as_dict() if relaxed_structure is not None else None
    # phonon_dos_doc = phonon_dos.as_dict() if phonon_dos is not None else None
    # phonon_bs_doc = phonon_bs.as_dict() if save_bandstructure and phonon_bs is not None else None

    # #Create a dictionary with the stability output
    # stability_output_doc = {
    #     "structure_index": structure_id,
    #     "relaxed_structure": relaxed_structure_doc,
    #     "phonon_dos": phonon_dos_doc,
    #     "phonon_bandstructure": phonon_bs_doc,
    # }

    # #Dump stability outputs to yaml file using monty serialization
    # dumpfn(stability_output_doc, filename_output)