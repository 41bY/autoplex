"""GenMLFF Jobs include the generation of the initial potential model as well as iterative GenMLFF exploration."""
...
# TODO: replace with "Generative Structure Search"
    do_randomized_structure_generation = BuildMultiRandomizedStructure(
        ...
        initial_selection_enabled=initial_selection_enabled, #Disable initial selection due to different outputs (list[paths]/list[structure])
        bcur_params=bcur_params,
        random_seed=random_seed,
    ).make()

# Static labelling calculations using pre-trained MLIP for testing and debuggin purposes
# Output of this job is a dict containing: list of paths to the labeled structures, list of config types (bulk|dimer|isolated atom)
...
# TODO: In the future this could work with multiple DFT-code (also pre-trained MLIP)
# TODO: This job could be performed by the login node
# Output is a dict = {"output_ref_dir" : path/to/all/labels.extxyz, "isolated_atom_energies" : {6: -243.0252124389}}
...
# TODO: This job could be performed by the login node
    do_data_preprocessing_ensemble = preprocess_data_ensemble(
        ...
        labeled_data_file=do_data_collection.output["output_ref_dir"],
