"""GenMLFF (Generative Model-Machine learning force field) flow for exploring and learning potential energy surfaces from scratch."""
...
@dataclass
class GenMLFFMaker(Maker):
    """
    Maker to set up and run GenMLFF for exploring and learning potential-energy surfaces (from scratch).
    ...
    rss_config: RssConfig
        Pydantic model that defines the setup parameters for the whole GenMLFF workflow.
        If not explicitly set, the defaults from 'autoplex.settings.RssConfig' will be used.
    """

    name: str = "ml-driven generative structure search"
    ...
    def make(self, **kwargs):
        ...
        **fit_kwargs:
        ...
        if config_params["train_from_scratch"]:
            ...
            initial_rss_job = initial_GenMLFF(**initial_params)
        ...
        if config_params["train_from_scratch"]:
            ...
            do_rss_job = do_GenMLFF_iterations(
